...
export const GET: RequestHandler = async ({ platform, params, url }) => {
	...
	try {
		...
		// Proxy thumbnail URLs through the admin endpoint
		const items = (result.mediaItems ?? []).map((item) => ({
			...item,
			thumbnailUrl: `/api/admin/integrations/google-photos/sessions/${params.sessionId}/items/thumbnail?baseUrl=${encodeURIComponent(item.mediaFile.baseUrl)}&type=${item.type}`
		}));
