...
{#if item}
  <div class="absolute left-0 right-0 bottom-0 bg-cream z-20 flex flex-col transition-all duration-300 ease-out" style="top: 120px; background-color: #F5F1EB;">
    <div class="p-8 flex-1 flex flex-col animate-fade-in bg-cream">
      ...
      <h3 class="font-display text-2xl font-normal text-walnut leading-tight mb-4" style="view-transition-name: hover-info-title;">
        {item.name}
      </h3>
      <p class="text-sm text-ash leading-relaxed mb-6" style="view-transition-name: hover-info-description;">
        {item.description}
      </p>
      {#if item.metadata}
        <div class="flex gap-8 mt-auto" style="view-transition-name: hover-info-meta;">
